...
class PFC2D_Vacancy:
  ...
  def InitParms(self):
    ...
    np.random.seed(np.uint64(self.parms.seed))
  ...
  def SetGeometry(self, NX, NY, PPUx, scalefactor=1, forceUnitCellBoundary=True):
    ...
    self.r = np.dstack((self.x, self.y))
    ...
    # Create noise mask.
    self.noiseMask = np.ones((self.ny, self.nx))
  ...
  def SetDT(self, dt):
    self.parms.dt = dt
    self.expcoeff = np.exp(self.lincoeff*self.parms.dt)

    self.expcoeff_nonlin = np.ones_like(self.x) * self.parms.dt
    self.expcoeff_nonlin[self.lincoeff != 0] = (self.expcoeff[self.lincoeff != 0] - 1)/self.lincoeff[self.lincoeff != 0]

    self.expcoeff_nonlin2 = np.zeros_like(self.x)
    self.expcoeff_nonlin2[self.lincoeff != 0] = (self.expcoeff[self.lincoeff != 0] - (1 + self.lincoeff[self.lincoeff != 0]*self.parms.dt))/np.power(self.lincoeff[self.lincoeff != 0],2)

  def GetEtaNoise(self):
    # return self.noise
    noisex = np.random.normal(loc=0, scale=1, size=(self.ny, self.nx))
    noisex_fft = 1j*self.kx*np.fft.fft2(noisex)
    noisey = np.random.normal(loc=0, scale=1, size=(self.ny, self.nx))
    noisey_fft = 1j*self.ky*np.fft.fft2(noisey)
    ...
    self.noise = np.fft.ifft2(self.noise_fft).real

    # renormalize to eta
    self.noise *= self.parms.eta/np.sqrt(np.power(self.noise, 2).mean())
    self.noise_fft = np.fft.fft2(self.noise)
    return self.noise

  def InitializeTimeSmoothNoise(self):
    self.noise_t = np.zeros((self.ny, self.nx, self.parms.NoiseTimeSmoothingFrames))
    for i in range(self.parms.NoiseTimeSmoothingFrames):
      self.noise_t[:, :, i] = self.GetEtaNoise()

    self.omega2 = np.fft.fftfreq(self.parms.NoiseTimeSmoothingFrames, d=1)**2  # d=dt?

  def GenerateTimeSmoothNoise(self):
    # self.noise_t[:,:,0:-1] = self.noise_t[:,:,1:]
    # self.noise_t[:,:,-1] = self.GetEtaNoise()
    self.noise_t = np.concatenate((self.noise_t[:,:,1:], np.array(self.GetEtaNoise()).reshape(self.ny, self.nx,1)), axis=2)
    # self.noise_t_hat = np.fft.fft2(self.noise_t)
    ...
    # self.noise_t = np.fft.ifft2(self.noise_t_hat).real
  ...
  def InitFieldFlat(self, noisy = True):
    self.phi = self.parms.phi0 * np.ones((self.nx, self.ny))
    self.phi += self.GetEtaNoise() if noisy & (self.parms.eta != 0) else 0
    self.phi_hat = np.fft.fft2(self.phi)
    self.phi0 = np.fft.ifft2(self.phi_hat).real
    self.t = 0
  ...
  def InitFieldCrystal(self, A = None, noisy = True, scalefactor=1):
    ...
    q1 = np.array([-np.sqrt(3)*0.5, -0.5])
    q2 = np.array([0.0, 1.0])
    q3 = np.array([np.sqrt(3)*0.5, -0.5])

    self.phi = 2 * A * (np.cos(self.r.dot(q1)/scalefactor) + np.cos(self.r.dot(q2)/scalefactor) + np.cos(self.r.dot(q3)/scalefactor)) + self.parms.phi0
    self.phi_hat = np.fft.fft2(self.phi)
    self.phi_hat[0,0] = self.nx*self.ny*self.parms.phi0
    self.phi = np.fft.ifft2(self.phi_hat).real
    self.phi += self.GetEtaNoise() if noisy & (self.parms.eta != 0) else 0
    self.phi_hat = np.fft.fft2(self.phi)
    self.phi_hat[0,0] = self.nx*self.ny*self.parms.phi0
    self.phi = np.fft.ifft2(self.phi_hat).real
  ...
  def TimeStepCross(self):
    ...
    self.phi0 = np.fft.ifft2(self.phi_hat0).real
    ...
    phi1 = np.fft.ifft2(phi_hat1).real
  ...
  def Get_N_hat(self, phi):
    # phi^2 term
    self.phi2 = self.parms.g * np.power(phi, 2)

    # phi^3 term
    self.phi3 = self.parms.v0 * np.power(phi, 3)

    # vacancy energy (stored as member for debugging)
    self.vac = -6*self.parms.Hng*np.power(phi,2)*(phi < 0) # 3 ϕ (|ϕ| - ϕ) = { -6ϕ^2 where ϕ < 0, or, 0 where ϕ > 0 }

    # nl energy (stored as member for debugging)
    self.ln = np.ones_like(phi) * self.minLog
    pos_mask = phi > -self.parms.a + np.exp(self.minLog)
    self.ln[pos_mask] = self.parms.Hln * np.log(phi[pos_mask] + self.parms.a)

    if self.parms.BoundaryPotential is not None:
      _VbMax = np.max(self.parms.BoundaryPotential)
    ...
    # if noise_dynamics is true, add noise to phi2 and phi3
    if self.parms.NoiseDynamicsFlag:
      return np.fft.fft2(self.phi2 + self.phi3 + self.vac + self.ln + self.boundary) + self.noise_fft

    return np.fft.fft2(self.phi2 + self.phi3 + self.vac + self.ln + self.boundary)

  def CalcEnergyDensity(self):
    self.phi_hat = np.fft.fft2(self.phi)
    self.energy_lin_1 = np.fft.ifft2(self.linenergycoeff * self.phi_hat).real
    self.energy_lin = self.phi * self.energy_lin_1

    self.energy_ln = np.zeros_like(self.phi)
    pos_mask = self.phi > -self.parms.a
    self.energy_ln[pos_mask] = self.parms.Hln * (self.phi[pos_mask] + self.parms.a) * np.log(self.phi[pos_mask] + self.parms.a)

    self.energy_poly = 1/2 * (self.parms.epsilon + self.parms.beta) * np.power(self.phi,2) + 1/3 * self.parms.g * np.power(self.phi,3) + 1/4 * self.parms.v0 * np.power(self.phi,4)
  ...
  def SPT_LocateCM(self, nxy = 10, min_distance=5, threshold_abs=0.8):
    # Find peaks in phi
    peaks = peak_local_max(self.phi, min_distance=min_distance, threshold_abs=threshold_abs, exclude_border=False)
    ...
    for i, (y_peak, x_peak) in enumerate(peaks):
      ...
      # extract region
      region = self.phi[y_begin:y_end, x_begin:x_end]
      mass = region.sum()
